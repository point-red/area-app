...
const {
  ...
  columns: {
    ...
    client_id: { label: 'Client ID', isVisible: true, isSelectable: true }
  }
})
 ...
 * - clients data retrieved from API
 * - loading state
 * - control flags to prevent unnecessary watcher triggers
 */
const clients = ref<IData[]>()
...
const onPageUpdate = async () => {
  if (!isInitialSetup.value) {
    await getClients(pagination.page)
 ...
 * @param client - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (client: IData, index: number) => {
...
<template>
  <base-card>
    ...
    <div class="my-5 flex gap-2">
      <!-- Button to navigate to create new client page -->
      ...
      <base-table>
        <thead>
          ...
          <tr class="bg-slate-100 dark:bg-slate-700">
            ...
            <th v-if="columns['client_id'].isVisible">
              <base-input v-model="filter.client_id" placeholder="Search..." :readonly="isLoading" border="none" />
            </th>
          </tr>
        </thead>
        ...
        <tbody>
          ...
          <!-- Show no data found message if no clients and query params exist -->
          <tr v-if="!isLoading && clients?.length === 0 && route.query">
            <td :colspan="countVisibleColumns + 1">
          ...
          <!-- Render rows of client data when available -->
          <template v-if="!isLoading && clients && clients.length > 0">
            <tr v-for="(client, index) in clients" :key="index">
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0!">
                      <div class="flex flex-col">
                        ...
                        <router-link :to="`/clients/${client._id}/edit`">
                          <base-button variant="text" color="info"
                            class="w-full py-1! px-3! m-0! flex items-center justify-start text-left!">
                        ...
                        <base-divider orientation="vertical" class="my-0!" />
                        <base-button @click="onDeleteModal(client, index)" variant="text" color="danger"
                          class="w-full py-1! px-3! m-0! flex items-center justify-start text-left!">
              ...
              <!-- Client fields rendered conditionally based on column visibility -->
              <td v-if="columns['name'].isVisible">
                <router-link :to="`/clients/${client._id}`" class="text-blue">{{ client.name }}</router-link>
              </td>
              <td v-if="columns['client_id'].isVisible">{{ client.client_id }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
