...
import { deleteClientApi } from './delete.api'
...
const onDelete = async () => {
  ...
  try {
    ...
    // start api call
    const responseDelete = await deleteClientApi(_id.value as string)
    if (!responseDelete) {
      form.data.password = ''
      return
    }
    emit('deleted')
    form.reset()
    toast(`Delete Client "${label.value}" success`, { color: 'success' })
    toggleModal(false)
  } catch (error) {
...
<template>
  <base-modal :is-open="showModal" @on-close="toggleModal(false)">
    <div class="max-h-90vh overflow-auto p-4">
      <h2 class="py-4 text-2xl font-bold">Delete Client</h2>
      <div class="space-y-8">
        <div>
