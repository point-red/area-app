...
import { deleteUserApi } from './delete.api'
...
const onDelete = async () => {
  ...
  try {
    ...
    // start api call
    const responseDelete = await deleteUserApi(_id.value as string)
    if (!responseDelete) {
      form.data.password = ''
      return
    }
    emit('deleted')
    form.reset()
    toast(`Delete User "${label.value}" success`, { color: 'success' })
    toggleModal(false)
  } catch (error) {
    ...
    <div class="max-h-90vh overflow-auto p-4">
      ...
      <div class="space-y-8">
        ...
        <div class="flex flex-col gap-2">
          ...
          <base-textuser label="Reason" v-model="form.data.reason" :errors="form.errors.reason" :min-height="64"
